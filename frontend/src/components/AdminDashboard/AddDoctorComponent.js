		...
		if (dialogType === "add") {
			try {
				const response = await axios.post(
					"http://localhost:5001/api/doctors/single",
					{ id_doctor: idDoctor, doctor_name: doctorName }
				);
			...
			try {
				const response = await axios.put(
					`http://localhost:5001/api/doctors/${editingDoctor._id}`,
					{ id_doctor: idDoctor, doctor_name: doctorName }
				);
				setDoctors(
					doctors.map((d) => (d._id === response.data._id ? response.data : d))
				);
					...
					<TableBody>
						{doctors.map((doctor) => (
							<TableRow key={doctor._id}>
								...
								<TableCell>
									<IconButton onClick={() => handleOpenDialog("edit", doctor)}>
										<EditIcon />
									</IconButton>
									<IconButton onClick={() => handleDeleteDoctor(doctor._id)}>
										<DeleteIcon />
									</IconButton>
								</TableCell>
							</TableRow>
